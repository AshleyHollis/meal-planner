...
async def _make_ingredient(session, name="Flour", category="pantry", default_unit="g") -> Ingredient:
...
async def _make_inventory_item(session, ingredient_id, household_id, quantity, unit="g") -> InventoryItem:
    now = datetime.now(UTC)
