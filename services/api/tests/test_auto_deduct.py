...
async def _make_inventory_item(
    session, ingredient_id, household_id, quantity, expiry_date, location="fridge"
) -> InventoryItem:
    now = datetime.now(UTC)
    item = InventoryItem(
        id=uuid4(),
...
async def test_deduct_oldest_expiry_first(session: AsyncSession, household):
    ...
    assert len(items) == 1
    assert (
        items[0].expiry_date.date()
        if hasattr(items[0].expiry_date, "date")
        else items[0].expiry_date == new_expiry.date()
    )
