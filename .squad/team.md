# Team Roster

> AI meal planner with inventory tracking, AI-powered weekly plans, and grocery lists — Next.js + FastAPI + Azure

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. Does not generate domain artifacts. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Dallas | Lead | `.squad/agents/dallas/charter.md` | ✅ Active |
| Ripley | Backend Dev | `.squad/agents/ripley/charter.md` | ✅ Active |
| Parker | DevOps | `.squad/agents/parker/charter.md` | ✅ Active |
| Lambert | Tester | `.squad/agents/lambert/charter.md` | ✅ Active |
| Scribe | Session Logger | `.squad/agents/scribe/charter.md` | 📋 Silent |
| Ralph | Work Monitor | — | 🔄 Monitor |

## Project Context

- **Owner:** Ashley Hollis
- **Stack:** Next.js 16, React 19, TypeScript 5, Tailwind CSS 4, FastAPI, SQLAlchemy 2.0, Python, Azure (AKS, SQL, SWA), Auth0, Playwright, GitHub Actions
- **Description:** AI-powered meal planner that tracks pantry inventory, generates weekly dinner plans using LLM, and produces consolidated grocery lists
- **Created:** 2026-03-02T08:27:00Z
- **Repo:** AshleyHollis/meal-planner
- **Branch:** 001-meal-planner-mvp
- **PR:** #1

## Issue Source

- **Repository:** AshleyHollis/meal-planner
- **Connected:** 2026-03-02T08:27:00Z
