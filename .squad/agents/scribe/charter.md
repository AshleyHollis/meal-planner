...
1. **Write orchestration log entries** to `.squad/orchestration-log/{timestamp}-{agent-name}.md`
...
6. **Archive decisions** if decisions.md exceeds ~20KB (entries older than 30 days → decisions-archive.md)
7. **Summarize history** if any agent's history.md exceeds ~12KB (old entries → ## Core Context)
8. **Commit `.squad/` changes** using temp file for commit message (Windows-compatible)
