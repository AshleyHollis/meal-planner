...
Tier 2 optimisation required converting the sequential multi-meal generation loop to
asyncio.gather()-based parallel execution. The existing `call_llm()` function is
synchronous (blocking HTTP via httpx) and is called from the async `_generate_with_retries()`.

## Decision

Wrap every `call_llm()` invocation in `asyncio.to_thread(call_llm, prompt)` inside
`_generate_with_retries()`. This is the **only** call site.

**Do not** call `call_llm()` directly from async context when multiple concurrent tasks
are in flight — doing so would block the event loop and defeat the parallelism.

## Rationale

- `asyncio.to_thread()` runs the blocking function in a thread-pool executor, freeing
  the event loop for other coroutines (including the other meal-type tasks).
- This is the minimal change: no rewrite of the HTTP layer, no migration to
  `AsyncAzureOpenAI`, no new dependencies.
- Full `AsyncAzureOpenAI` migration remains a follow-on (Tier 3) if needed.

## Constraints

- Semaphore limit of 2 (`MAX_PARALLEL_LLM_CALLS = 2`) matches 20K TPM / ~5K tokens
  per call = ~4 theoretical max; 2 is conservative and safe for production.
- 2-second index-based stagger (`2 * index`) prevents simultaneous burst on the
  Azure AI Foundry token bucket.
