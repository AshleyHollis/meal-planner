...
3. Send a "session started" notification to Discord `#meal-planner` (channel `1479061992772997202`):
   ...
   ```
4. Wait 15 seconds for the watcher to connect and do its first poll, then check inbox again for any messages sent while offline that the watcher just picked up:
   ```powershell
...
**Architectural limitation**: The agent cannot continuously poll inbox.json in the background. It can only check at discrete points (start of message processing, between tasks, before task_complete). The user should send a CLI message if they need immediate attention; Discord messages will be picked up at the next check point.
