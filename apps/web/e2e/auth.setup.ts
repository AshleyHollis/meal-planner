...
setup("authenticate as test user", async ({ page }) => {
  ...
  if (!email || !password) {
    console.warn(
      "[auth-setup] Test credentials not set. Skipping authentication.",
    );
    console.warn(
  ...
  try {
    ...
    const errorElement = page
      .locator('.error-page, [class*="error"], #error-message')
      .first();
    try {
    ...
    try {
      ...
      console.log(
        "[auth-setup] Email field not found (may be pre-filled or identifier-first)",
      );
    }
    ...
    if (!passwordVisible) {
      console.log(
        "[auth-setup] Password not visible — trying identifier-first flow",
      );
      const continueBtn = page
        .locator(
          'button[name="action"], button[type="submit"], button[data-action-button-primary]',
        )
        .first();
      try {
        ...
        console.log(
          "[auth-setup] Continue button not found or password still not visible",
        );
      }
    }
    ...
    if (!passwordVisible) {
      ...
      console.error(
        `[auth-setup] Page content (first 2000 chars): ${pageContent.substring(0, 2000)}`,
      );
