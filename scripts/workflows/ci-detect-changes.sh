...
#
# Inputs:
#   - $MAIN_BRANCH: 'true' if master branch, 'false' if PR
#
# Outputs:
...
# Always run full validation for master branch, forced full runs, or pull_request events
if [[ "$main_branch" == "true" || "$force_full" == "true" || "$event_name" == "pull_request" ]]; then
  ...
  if [[ "$event_name" == "pull_request" ]]; then
    ...
    echo "✓ Master branch: All validation jobs will run"
  fi
else
