...
#   1. Execution mode (master branch vs PR)
...
# Format and append rationale to workflow summary
{
  ...
  echo ""
  echo "- Mode: $([[ "$IS_MAIN_BRANCH" == "true" ]] && echo "master branch (full validation)" || echo "PR (change-based)")"
  echo "- Changed areas: $CHANGED_AREAS"
  echo "- Lint Python: $LINT_PYTHON_RESULT (reason: $([[ "$IS_MAIN_BRANCH" == "true" ]] && echo "master branch" || (echo "$CHANGED_AREAS" | grep -qE "(services/api|services/workers|services/shared)" && echo "python changes" || echo "no python changes")))"
  echo "- Lint Frontend: $LINT_FRONTEND_RESULT (reason: $([[ "$IS_MAIN_BRANCH" == "true" ]] && echo "master branch" || (echo "$CHANGED_AREAS" | grep -q "apps/web" && echo "frontend changes" || echo "no frontend changes")))"
  ...
  echo "- Workflow Validation: $VALIDATE_WORKFLOWS_RESULT (reason: $([[ "$IS_MAIN_BRANCH" == "true" ]] && echo "master branch" || (echo "$CHANGED_AREAS" | grep -q "ci" && echo "ci/workflow changes" || echo "no ci changes")))"
} >> "$GITHUB_STEP_SUMMARY"
