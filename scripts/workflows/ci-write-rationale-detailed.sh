...
#   IS_MAIN_BRANCH - true if running on master branch
...
#
# LOGIC:
#   1. Determine execution mode (master branch vs PR)
...
if [[ "${IS_MAIN_BRANCH}" == "true" ]]; then
  MODE="master branch (full validation)"
else
...
if [[ "${IS_MAIN_BRANCH}" == "true" ]]; then
  LINT_PYTHON_REASON="master branch"
...
if [[ "${IS_MAIN_BRANCH}" == "true" ]]; then
  LINT_FRONTEND_REASON="master branch"
...
if [[ "${IS_MAIN_BRANCH}" == "true" ]]; then
  VALIDATE_WORKFLOWS_REASON="master branch"
