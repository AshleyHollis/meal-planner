...
if [ -n "$IMAGE_TAG" ] || [ "$BUILD_IMAGES_RESULT" != "skipped" ]; then
  ...
  if [ -n "$IMAGE_TAG" ]; then
    ...
    echo "- \`acrytsummprd.azurecr.io/meal-planner-api:$IMAGE_TAG\`" >> $GITHUB_STEP_SUMMARY
    echo "- \`acrytsummprd.azurecr.io/meal-planner-workers:$IMAGE_TAG\`" >> $GITHUB_STEP_SUMMARY
  fi
  echo "" >> $GITHUB_STEP_SUMMARY
fi
